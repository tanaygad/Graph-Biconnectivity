...
void add_edge (int a1, int a2, int b1, int b2) {
    // int w = (*dsu).find_set(a1), x = (*dsu).find_set(a2), y = (*dsu).find_set(b1), z = (*dsu).find_set(b2);
    // if (w==x && w == y && w==z) return;
    int i1 = edgeadded[{min(a1,a2),max(a1,a2)}];
    int i2 = edgeadded[{min(b1,b2),max(b1,b2)}];
    if ((*dsu).find_set(i1)==(*dsu).find_set(i2)) return;

    #pragma omp critical
    {
    (*dsu).union_sets(i1,i2);
